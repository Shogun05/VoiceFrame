...
class VoiceSynthesizer:
    ...
    def synthesize_dialogues(self, dialogues: list, characters: list):
        ...
        """
        print(f"Starting voice synthesis for {len(dialogues)} dialogues")
        ...
        for i, dlg in enumerate(dialogues):
            try:
                ...
                print(f"Processing dialogue {i+1}: {canonical_name} ({gender}) - {line[:30]}...")
                
                voice = self._get_voice_for_character(cache_key, gender)

                # Create simple sequential filename for video_gen compatibility
                out_file = os.path.join(self.output_dir, f"{i+1}.wav")
                print(f"Generating audio file: {out_file}")
                ...
                if os.path.exists(out_file):
                    ...
                    print(f"Successfully created {out_file} ({file_size} bytes)")
                else:
                    print(f"ERROR: File {out_file} was not created!")
                    
            except Exception as e:
                print(f"ERROR generating audio for dialogue {i+1}: {e}")
                import traceback
                traceback.print_exc()
                continue
        ...
        print(f"Voice synthesis complete. Created {len(audio_files)} audio files: {audio_files}")
